...
/// <summary>
/// Chuyển đổi WinForms Keys enum sang (Key, Code) theo chuẩn Web KeyboardEvent API.
/// - Key  = tên logic của phím (e.g. "Enter", "a", "F5", "ArrowLeft")
/// - Code = mã vị trí vật lý  (e.g. "Enter", "KeyA", "F5", "ArrowLeft")
///
/// Hỗ trợ phân biệt Left/Right cho Ctrl, Shift, Alt (LControlKey vs RControlKey...).
/// </summary>
public static class KeyMapper
{
    /// <summary>
    /// Map WinForms Keys → (Key, Code).
    /// Gọi với keyCode đã strip modifier (& Keys.KeyCode) để tránh nhầm.
    /// </summary>
    public static (string Key, string Code) Map(Keys keyCode)
    {
        // Strip modifier bits — chỉ lấy phần mã phím thực
        ...
        return key switch
        {
            ...
            //  Modifier — phân biệt Left / Right 
            // QUAN TRỌNG: phải kiểm tra LControlKey/RControlKey TRƯỚC ControlKey
            // vì WinForms có thể trả về cả hai.
            Keys.LControlKey => ("Control", "ControlLeft"),
            Keys.RControlKey => ("Control", "ControlRight"),
            Keys.ControlKey  => ("Control", "ControlLeft"),  // fallback generic

            Keys.LShiftKey   => ("Shift", "ShiftLeft"),
            Keys.RShiftKey   => ("Shift", "ShiftRight"),
            Keys.ShiftKey    => ("Shift", "ShiftLeft"),      // fallback generic

            Keys.LMenu       => ("Alt", "AltLeft"),
            Keys.RMenu       => ("Alt", "AltRight"),
            Keys.Menu        => ("Alt", "AltLeft"),          // fallback generic

            Keys.LWin        => ("Meta", "MetaLeft"),
            Keys.RWin        => ("Meta", "MetaRight"),
